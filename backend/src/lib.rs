...
/// standalone run. `None` = send no bearer; Shadow then rejects the call (401)
/// and the fail-soft handlers surface `{ available: false }`.
pub(crate) fn shadow_token() -> Option<String> {
...
/// Attach the Shadow bearer to a request when a token is available. Extension
/// trait so every proxied Shadow hop in `api.rs` is a one-word addition and the
/// token resolution can never drift per call site.
pub(crate) trait ShadowAuth {
    fn shadow_auth(self) -> Self;
}

impl ShadowAuth for reqwest::RequestBuilder {
    fn shadow_auth(self) -> Self {
        match shadow_token() {
