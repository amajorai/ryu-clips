...
fn spawn_event(ctx: &ClipsCtx, event: &'static str, payload: Value) {
    spawn_event_with_notify(ctx, event, payload, None);
}

/// [`spawn_event`] plus an optional user-facing notification raised alongside the
/// fan-out (the Inbox shows a clip-ready row with the Clips icon).
...
fn announce_clip_ready(ctx: &ClipsCtx, bundle: &Value, source: &'static str) {
    ...
    spawn_event_with_notify(
        ...
        // A finished clip is exactly the "long job done" moment a notification exists
        // for — announce it even when nobody is watching the app.
        Some(ryu_app_events::NotifyHint::info(
            format!("Clip ready: {title}"),
            Some(format!("{:.0}s recording", duration_ms as f64 / 1000.0)),
        )),
    );
}
