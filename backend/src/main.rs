...
async fn main() -> anyhow::Result<()> {
    ...
    // endpoint is what makes the whole `/api/clips` surface callable by an agent
    // (`ryu_clips::api::openapi()` was dead code until now — only tests read it).
    //
    // Root, not under `/api/clips`: Core tries the root FIRST and only falls back to
    ...
    let clips = Router::new()
        ...
        .route(
            "/openapi.json",
            get(|| async { Json(ryu_clips::api::openapi()) }),
        )
...
#[cfg(test)]
mod tests {
    use super::{bearer_ok, ct_eq, health};
    ...
    #[test]
    fn ct_eq_compares_length_then_contents() {
        assert!(ct_eq(b"", b""));
        assert!(ct_eq(b"abc", b"abc"));
        assert!(!ct_eq(b"abc", b"abd"));
        // Length mismatch short-circuits to false (prefix is NOT a match).
        assert!(!ct_eq(b"ab", b"abc"));
        assert!(!ct_eq(b"abc", b"ab"));
    }

    #[tokio::test]
    async fn sidecar_host_degrades_ytdlp_and_download_cleanly() {
        use super::SidecarClipsHost;
        use ryu_clips::ClipsHost;
        let host = SidecarClipsHost;
        // The two Core-kernel-coupled operations degrade with a clear Err...
        let e = host.ensure_ytdlp().await.unwrap_err();
        assert!(e.contains("URL ingest"));
        let e = host
            .download_video("https://x/v", std::path::Path::new("/tmp"), None, None)
            .await
            .unwrap_err();
        assert!(e.contains("URL ingest"));
        // ...tmp_dir is the one thing it honestly owns (under the data dir)...
        assert!(host.tmp_dir().ends_with("tmp"));
        // ...and Space filing degrades to a log (no panic, no error channel).
        host.store_clip("Demo", Some(vec![1, 2]), "# Demo").await;
    }

    #[tokio::test]
    async fn health_is_ok_and_static() {
